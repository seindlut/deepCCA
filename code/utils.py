...
def load_data_full():
    '''
    Loads the dataset (full size image)
    '''
    ...
    shared_train_x = theano.shared(numpy.asarray(train_set_x,dtype=theano.config.floatX),borrow=True)
    shared_train_y = theano.shared(numpy.asarray(train_set_y,dtype=theano.config.floatX),borrow=True)
    shared_test_x = theano.shared(numpy.asarray(test_set_x,dtype=theano.config.floatX),borrow=True)
    shared_test_y = theano.shared(numpy.asarray(test_set_y,dtype=theano.config.floatX),borrow=True)

    rval = [(shared_train_x, shared_train_y), (shared_test_x, shared_test_y)]
    print 'Done'
    return rval
